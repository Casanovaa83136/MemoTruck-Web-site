...
- **`prefers-reduced-motion: reduce` is respected** — a dedicated media query block at the end of the stylesheet disables entrance animations, the marquee scroll, the radar ping, and the grain overlay for users who request it. Any new animation should be added to that block too.
